...
class Sidebar extends React.Component {
  ...
  render() {
    ...
    let disabled =
      (request.init && !request.completed) ||
      this.props.editor.code.trim() === "";
    let output = null,
      status = null,
      stderr = null,
      comp_out = null;
    if (!request.init) {
      ...
      if (request.output) status = request.output.status.description;
      else status = "Queued";
    } else {
      output = "loading...";
    }
        ...
        <fieldset disabled={disabled}>
          <Form onSubmit={this.handleSubmit.bind(this)} className="w-100">
            <Form.Group className="mb-1" controlId="stdin">
              <Form.Label>
                <strong className="text-white">Input (stdin)</strong>
              </Form.Label>
              <Form.Control
                onChange={this.handleStdin.bind(this)}
                name="stdin"
                as="textarea"
                rows="10"
          ...
          <Card className="mt-4" bg="dark" text="white">
            ...
            <Card.Body>
              ...
              <Card.Footer className="font-italic text-info">
                {status}
              </Card.Footer>
            </Card.Body>
          </Card>
...
export default connect(mapStateToProps, {changeStdin, submitCode, checkResult})(
  Sidebar
);
